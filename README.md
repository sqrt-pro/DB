...
* Сеттеры поддерживают указание даты в любом формате, поддерживаемом функцией `strtotime()`
  ~~~ php
  $item->setCreatedAt('2015-01-01 12:45');
  $item->setCreatedAt('-7 days');
  ~~~

* Геттеры поддерживают форматирование в формате, принимаемом функцией date()
  ~~~
